...
<div class="max-w-2xl mx-auto animate-fade-in-up">
    <div class="flex items-center mb-6 space-x-4">
        ...
        <h1 class="text-3xl font-extrabold text-solarized-base01">Edit Pengguna</h1>
    </div>
            ...
            <form action="{{ route('pengguna.update', $pengguna->user_id) }}" method="POST">
                ...
                <div class="grid grid-cols-1 gap-y-6">
                    <div>
                        ...
                        <input type="text" name="username" id="username" required value="{{ old('username', $pengguna->username) }}"
                    ...
                    <div>
                        <label for="password" class="block text-base font-semibold text-solarized-base00 mb-1">Password Baru (Opsional)</label>
                    ...
                    <div>
                        ...
                        <select name="role" id="role" required
                            ...
                            <option value="PA/KPA" {{ old('role', $pengguna->role) == 'PA/KPA' ? 'selected' : '' }}>PA/KPA</option>
                            <option value="PPK" {{ old('role', $pengguna->role) == 'PPK' ? 'selected' : '' }}>PPK</option>
                            <option value="Pokja" {{ old('role', $pengguna->role) == 'Pokja' ? 'selected' : '' }}>Pokja</option>
                            <option value="Pejabat Pengadaan" {{ old('role', $pengguna->role) == 'Pejabat Pengadaan' ? 'selected' : '' }}>Pejabat Pengadaan</option>
                        </select>
                        @error('role')
                            <p class="mt-2 text-sm text-solarized-red font-medium">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <select name="status_aktif" id="status_aktif" required
                            ...
                            <option value="1" {{ old('status_aktif', $pengguna->status_aktif) == '1' ? 'selected' : '' }}>Aktif</option>
                            <option value="0" {{ old('status_aktif', $pengguna->status_aktif) == '0' ? 'selected' : '' }}>Nonaktif</option>
                        </select>
                        @error('status_aktif')
                            <p class="mt-2 text-sm text-solarized-red font-medium">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <select name="personil_id" id="personil_id"
                            ...
                            @foreach($personils as $personil)
                                <option value="{{ $personil->personil_id }}" {{ old('personil_id', $pengguna->personil_id) == $personil->personil_id ? 'selected' : '' }}>
